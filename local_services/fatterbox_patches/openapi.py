...
class TTSRequest(BaseModel):
    ...
    language_id: Optional[str] = Field(None, description="Language code (e.g. 'ru', 'en')")
...
def create_api(model, voices: dict, voices_dir: Path) -> FastAPI:
    ...
    async def synthesize(request: TTSRequest):
        ...
        """
        text = request.input
        _LOGGER.info(f"TTS request: '{text[:50]}...' with voice: {request.voice}, lang: {request.language_id}")
...
def _generate_audio_sync(model, text: str, audio_prompt_path: str = None, language_id: str = None) -> torch.Tensor:
    ...
    with torch.no_grad():
        ...
        wav = model.generate(
            text,
            language_id, # Second positional argument
...
def _create_wav_header(sample_rate: int, channels: int = 1, sample_width: int = 2) -> bytes:
    ...
    return (riff + file_size + wave + fmt + fmt_size + audio_format + num_channels + 
            sample_rate_bytes + byte_rate + block_align + bits_per_sample + data + data_size)
